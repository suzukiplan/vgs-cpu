...
int test_ld_c_literal(struct vgscpu_context* c)
{
    ...
    vgscpu_run(c);
    if (c->r.c != 0x000000CD) FAILED(TAG, __LINE__);

    vgscpu_load_program(c, op2, sizeof(op2));
    vgscpu_run(c);
    if (c->r.c != 0x0000CDAB) FAILED(TAG, __LINE__);

    vgscpu_load_program(c, op3, sizeof(op3));
    vgscpu_run(c);
    if (c->r.c != 0xCDAB8967) FAILED(TAG, __LINE__);
    return 0;
}
...
int test_ld_c_registry(struct vgscpu_context* c)
{
    ...
    c->r.d = 0x090a0b0c;
    vgscpu_run(c);
    if (c->r.c != 0x01020304) FAILED(TAG, __LINE__);
    ...
    c->r.d = 0x090a0b0c;
    vgscpu_run(c);
    if (c->r.c != 0x05060708) FAILED(TAG, __LINE__);
    ...
    c->r.d = 0x090a0b0c;
    vgscpu_run(c);
    if (c->r.c != 0x090a0b0c) FAILED(TAG, __LINE__);
    return 0;
}
...
int test_ld_c_memory(struct vgscpu_context* c)
{
    ...
    vgscpu_run(c);
    if (c->r.c != 0x00000008) FAILED(TAG, __LINE__);

    vgscpu_load_program(c, op2, sizeof(op2));
    memcpy(&c->m, m, sizeof(m));
    vgscpu_run(c);
    if (c->r.c != 0x00000908) FAILED(TAG, __LINE__);

    vgscpu_load_program(c, op3, sizeof(op3));
    memcpy(&c->m, m, sizeof(m));
    vgscpu_run(c);
    if (c->r.c != 0x0b0a0908) FAILED(TAG, __LINE__);
    return 0;
}
